...
app.patch("/tokens/phone/:phone", async (req, res) => {
  //
});
...
app.post("/tokens/phone", async function (req, res) {
  ...
  // // 만약 해당 핸드폰번호가 이미 Tokens문서에 저장돼있다면.
  if (Tokens.find({ phone: myphone })) {
    // 생성한 인증 토큰으로 update
    await Tokens.update({ phone: myphone }, { token: mytoken });
    console.log("인증 토큰 업데이트함");
  }
  ...
  // Tokens문서에 저장되어있지 않다면, DB에 새로 저장
  await tokens.save();
