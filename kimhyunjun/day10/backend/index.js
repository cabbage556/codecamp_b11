...
app.post("/tokens/phone", async function (req, res) {
  ...
  const overLap = await checkPhones.find({phone: myphone})
...
app.patch("/tokens/phone", async function(req, res) {
  // 다시 시작
  const {phoneNb, tokenNb} = req.body
  const userTokenNumber = tokenNb
  const dbTokenSave = await checkPhones.find({phoneNb})

    if(dbTokenSave === null){
      res.send("false")
    }
  
    if(dbTokenSave.tokenNb === userTokenNumber) {
      await checkPhones.update({phoneNb},
        {isAuth : true})
        res.send("true")
    } else {
      await checkPhones.update({phoneNb},
